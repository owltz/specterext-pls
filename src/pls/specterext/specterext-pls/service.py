...
class SpecterextPlsService(Service):
    id = "specterext-pls"
    name = "Private Law Society"
    icon = "specterext-pls/img/pls.png"
    logo = "specterext-pls/img/pls.png"
    desc = "Private Contracts with Arbitrators."
    has_blueprint = True
    blueprint_module = "pls.specterext.specterext-pls.controller"
    ...
    @classmethod
    def get_associated_device(cls) -> Device:
        """Get the Specter `Device` that is currently associated with this service"""
        service_data = cls.get_current_user_service_data()
        if not service_data or cls.SPECTER_DEVICE_ALIAS not in service_data:
            # Service is not initialized; nothing to do
            return
        try:
            return app.specter.device_manager.get_by_alias(
                service_data[cls.SPECTER_DEVICE_ALIAS]
            )
        except SpecterError as e:
            logger.debug(e)
            # Referenced an unknown device
            # TODO: keep ignoring or remove the unknown device from service_data?
            return

    @classmethod
    def set_associated_device(cls, device: Device):
        """Set the Specter `Device` that is currently associated with this Service"""
        cls.update_current_user_service_data({cls.SPECTER_DEVICE_ALIAS: (device.alias if device else None)})
